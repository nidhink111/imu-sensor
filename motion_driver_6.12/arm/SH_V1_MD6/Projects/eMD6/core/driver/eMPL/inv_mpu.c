...
/**
 * Block for SeeHow MPL Target
 */
#elif defined EMPL_TARGET_SHS
#include "esp_mpl_support.h"
...
{
	SEGGER_RTT_printf(0,"mpu_init ENTER");

    unsigned char data[6];

#ifdef EMPL_TARGET_SHS
    SEGGER_RTT_printf(0,"Init 1\n");
    reg_int_cb(int_param);
    SEGGER_RTT_printf(0,"Init 2\n");
#endif
   ...
   SEGGER_RTT_printf(0,"Init 4\n");
    ...
    if (mpu_set_gyro_fsr(2000))
        return -1;
    SEGGER_RTT_printf(0,"Init 5\n");

    if (mpu_set_accel_fsr(16))
        return -1;
    SEGGER_RTT_printf(0,"Init 6\n");

    if (mpu_set_lpf(42))
        return -1;
    SEGGER_RTT_printf(0,"Init 7\n");

    if (mpu_set_sample_rate(50))
        return -1;
    SEGGER_RTT_printf(0,"Init 8\n");

    if (mpu_configure_fifo(0))
        return -1;
    SEGGER_RTT_printf(0,"Init 9\n");
...
#ifdef AK89xx_SECONDARY
    if(setup_compass()){
    	return SH_ERROR_COMPASS_INIT_FAILED;
    }
    SEGGER_RTT_printf(0,"Init 10.1\n");
    if (mpu_set_compass_sample_rate(10))
        return -1;
    SEGGER_RTT_printf(0,"Init 10.2\n");
#else
    /* Already disabled by setup_compass. */
    if (mpu_set_bypass(0))
        return -1;
    SEGGER_RTT_printf(0,"Init 10\n");
#endif

    mpu_set_sensors(0);
    SEGGER_RTT_printf(0, "mpu_init : st.chip_cfg.sensors = 0x%02X\n",st.chip_cfg.sensors);
    return 0;
}
...
AKM_restore:
    ...
    mpu_set_bypass(0);
    SEGGER_RTT_printf(0,"compass_self_test result = %d\n", result);
    return result;
}
#endif
...
int mpu_set_dmp_state(unsigned char enable)
{
    ...
    if (st.chip_cfg.dmp_on == enable) {
    		SEGGER_RTT_printf(0,"mpu_set_dmp_state : Already enabled ?\n");
        return 0;
    }

    if (enable) {
        if (!st.chip_cfg.dmp_loaded) {
        		SEGGER_RTT_printf(0,"mpu_set_dmp_state : ERROR: DMP is not loaded yet.\n");
            return -1;
        }
        SEGGER_RTT_printf(0,"mpu_set_dmp_state : Enable\n");
        ...
        mpu_reset_fifo();
    		...
    		SEGGER_RTT_printf(0,"mpu_set_dmp_state : Disable\n");
...
static int setup_compass(void)
{
    ...
    SEGGER_RTT_printf(0,"Init compass 1\n");
    ...
    SEGGER_RTT_printf(0,"Init compass 2\n");

    if (akm_addr > 0x0F) {
        /* TODO: Handle this case in all compass-related functions. */
        SEGGER_RTT_printf(0, "Compass not found.\n");
        return -1;
    }

    SEGGER_RTT_printf(0,"Init compass 3\n");
    ...
    if (i2c_write(st.chip_cfg.compass_addr, AKM_REG_CNTL, 1, data))
        return -1;
    delay_ms(1);
    SEGGER_RTT_printf(0,"Init compass 4\n");

    data[0] = AKM_FUSE_ROM_ACCESS;
    if (i2c_write(st.chip_cfg.compass_addr, AKM_REG_CNTL, 1, data))
        return -1;
    delay_ms(1);
    SEGGER_RTT_printf(0,"Init compass 5\n");

    /* Get sensitivity adjustment data from fuse ROM. */
    if (i2c_read(st.chip_cfg.compass_addr, AKM_REG_ASAX, 3, data))
        return -1;
    SEGGER_RTT_printf(0,"Init compass 6\n");
    ...
    if (i2c_write(st.chip_cfg.compass_addr, AKM_REG_CNTL, 1, data))
        return -1;
    delay_ms(1);
    SEGGER_RTT_printf(0,"Init compass 7\n");
    ...
    if (i2c_write(st.hw->addr, st.reg->i2c_mst, 1, data))
        return -1;
    SEGGER_RTT_printf(0,"Init compass 8\n");

    /* Slave 0 reads from AKM data registers. */
    data[0] = BIT_I2C_READ | st.chip_cfg.compass_addr;
    if (i2c_write(st.hw->addr, st.reg->s0_addr, 1, data))
        return -1;
    SEGGER_RTT_printf(0,"Init compass 9\n");

    /* Compass reads start at this register. */
    data[0] = AKM_REG_ST1;
    if (i2c_write(st.hw->addr, st.reg->s0_reg, 1, data))
        return -1;
    SEGGER_RTT_printf(0,"Init compass 10\n");

    /* Enable slave 0, 8-byte reads. */
    data[0] = BIT_SLAVE_EN | 8;
    if (i2c_write(st.hw->addr, st.reg->s0_ctrl, 1, data))
        return -1;
    SEGGER_RTT_printf(0,"Init compass 11\n");

    /* Slave 1 changes AKM measurement mode. */
    data[0] = st.chip_cfg.compass_addr;
    if (i2c_write(st.hw->addr, st.reg->s1_addr, 1, data))
        return -1;
    SEGGER_RTT_printf(0,"Init compass 12\n");

    /* AKM measurement mode register. */
    data[0] = AKM_REG_CNTL;
    if (i2c_write(st.hw->addr, st.reg->s1_reg, 1, data))
        return -1;
    SEGGER_RTT_printf(0,"Init compass 13\n");

    /* Enable slave 1, 1-byte writes. */
    data[0] = BIT_SLAVE_EN | 1;
    if (i2c_write(st.hw->addr, st.reg->s1_ctrl, 1, data))
        return -1;
    SEGGER_RTT_printf(0,"Init compass 14\n");

    /* Set slave 1 data. */
    data[0] = AKM_SINGLE_MEASUREMENT;
    if (i2c_write(st.hw->addr, st.reg->s1_do, 1, data))
        return -1;
    SEGGER_RTT_printf(0,"Init compass 15\n");

    /* Trigger slave 0 and slave 1 actions at each sample. */
    data[0] = 0x03;
    if (i2c_write(st.hw->addr, st.reg->i2c_delay_ctrl, 1, data))
        return -1;
    SEGGER_RTT_printf(0,"Init compass 16\n");
    ...
    if (i2c_write(st.hw->addr, st.reg->yg_offs_tc, 1, data))
        return -1;
    SEGGER_RTT_printf(0,"Init compass 16.1\n");
#endif
